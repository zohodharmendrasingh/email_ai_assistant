...
// Static files served by Catalyst Hosting, not the function
...
app.get("/auth/zoho/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(`/app/index.html?${redirectParams.toString()}`);
  } catch (error) {
...
app.get("/message/:folderId/:messageId", async (req, res) => {
  ...
  try {
    ...
    let html = detail.data?.content ?? "";
    const hasAttachments = detail.data?.hasAttachment ?? false;

    // Rewrite Zoho inline image URLs to include access_token as query param
    // so the browser can load them directly from Zoho without a proxy
    const activeToken = token ?? zohoAccessToken;
    html = html.replace(/src=["'](\/mail\/ImageDisplay\?[^"']+)["']/gi, (_m, path) => {
      const cleanPath = path.replace(/&amp;/gi, "&");
      const sep = cleanPath.includes("?") ? "&" : "?";
      return `src="https://mail.zoho.com${cleanPath}${sep}access_token=${activeToken}"`;
    });
...
app.post("/agent", async (req, res) => {
  ...
  try {
    ...
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1024,messages:[{role:"user",content:`Email assistant for Alfonso at Metwall. Lanes: ${ls}\nEmails:\n${list}\nInstruction: "${message}"\nReply ONLY with valid JSON no markdown: {"actions":[{"type":"move","emailId":"...","toLane":"..."}],"message":"brief confirmation"}`}]})
    });
    const d = await r.json();
...
app.post("/refine-draft", async (req, res) => {
  ...
  try {
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      method:"POST",
      headers:{"Content-Type":"application/json","x-api-key":process.env.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},
      body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1024,messages:[{role:"user",content:`You are refining an email draft for Alfonso Mendez, President & CEO at Metwall.
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      const retry = await fetch(url, {
        ...
        body: JSON.stringify({ mode: isRead ? "markAsRead" : "markAsUnread", messageId: [messageId], folderId: folderId })
      });
